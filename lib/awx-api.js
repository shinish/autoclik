...
async function getAwxConfig() {
  ...
  if (baseUrl) {
    ...
    // Add /api/v2 if not already present
...
/**
 * Test AWX connection and token validity
 * @param {string} customBaseUrl - Optional custom base URL
 * @param {string} customToken - Optional custom token
 * @returns {Promise<{valid: boolean, error?: string}>}
 */
export async function testAwxConnection(customBaseUrl = null, customToken = null) {
  try {
    ...
    if (isPlaceholder || !baseUrl) {
      return { valid: false, error: 'AWX_NOT_CONFIGURED', message: 'AWX Base URL is not configured' };
    }

    if (!token) {
      return { valid: false, error: 'AWX_TOKEN_MISSING', message: 'AWX Token is not configured' };
    }

    // Test the connection with ping endpoint
    ...
    if (error.response?.status === 401) {
      return { valid: false, error: 'AWX_INVALID_TOKEN', message: 'AWX Token is invalid or expired' };
    } else if (error.code === 'ECONNREFUSED') {
      return { valid: false, error: 'AWX_CONNECTION_REFUSED', message: 'Cannot connect to AWX server' };
    } else if (error.code === 'ENOTFOUND') {
      return { valid: false, error: 'AWX_NOT_FOUND', message: 'Cannot resolve AWX server hostname' };
    } else if (error.code === 'ETIMEDOUT') {
      return { valid: false, error: 'AWX_TIMEOUT', message: 'Connection to AWX server timed out' };
    }
    return { valid: false, error: 'AWX_CONNECTION_ERROR', message: error.message || 'Failed to connect to AWX' };
  }
}
...
export async function launchJobTemplate(templateId, customBody = {}, customBaseUrl = null, customToken = null) {
  ...
  // Check if AWX is properly configured
  const isPlaceholder = !baseUrl || baseUrl === 'https://awx.example.com/api/v2' || baseUrl.includes('example.com');

  if (isPlaceholder || !baseUrl) {
    const error = new Error('AWX_NOT_CONFIGURED');
    error.userMessage = 'AWX is not configured. Please configure AWX Base URL in Settings.';
    logError('🔧 AWX Base URL Not Configured', {
      '⚠️ Issue': 'No AWX base URL configured',
      '🔧 Template ID': templateId,
      '💡 Solution': 'Go to Settings page and configure your AWX base URL',
      '📍 Where to Fix': 'Settings > AWX Configuration > Base URL',
    });
    throw error;
  }
  ...
  if (!token) {
    const error = new Error('AWX_TOKEN_MISSING');
    error.userMessage = 'AWX token is not configured. Please set your AWX authentication token in Settings.';
  ...
  logInfo('🚀 Preparing to launch AWX job', {
    '🌐 AWX Server': baseUrl,
    '🔧 Template ID': templateId,
    '🔑 Token': 'Configured ✓',
    '📤 Payload': customBody,
